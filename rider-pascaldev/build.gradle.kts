...
val resharperPluginPath = File(repoRoot, "PascalDev")
val buildConfiguration = ext.properties["BuildConfiguration"] ?: "Debug"

val libFiles = listOf<String>(
        "PascalDev/bin/$buildConfiguration/net461/Antlr4.Runtime.Standard.dll")

val pluginFiles = listOf(
        "PascalDev/bin/$buildConfiguration/net461/JetBrains.ReSharper.Plugins.PascalDev")
...
val riderPascalDevTargetsGroup = "rider-pascaldev"
...
configure<RdgenParams> {
    val csOutput = File(repoRoot, "PascalDev/src/PascalDev/src/Protocol")
    val ktOutput = File(repoRoot, "rider-pascaldev/src/main/java/com/jetbrains/rider/plugins/pascaldev/protocol")
    ...
    sources(File(repoRoot, "rider-pascaldev/protocol/src/kotlin/model"))
    packages = "model"
...
tasks {
    ...
    create("writeRiderSdkVersionProps") {
        group = riderPascalDevTargetsGroup
        doLast {
...
""")
    ...
    create("prepare") {
        ...
        doLast {
            exec {
                executable = "dotnet"
                args = listOf("restore", "$resharperPluginPath/PascalDev.sln")
            }
        }
    }
    ...
    create("buildReSharperPlugin") {
        ...
        doLast {
            exec {
                executable = "msbuild"
                args = listOf("$resharperPluginPath/PascalDev.sln")
            }
        }
    }
